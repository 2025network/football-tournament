...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full overflow-visible border-b border-white/10 bg-slate-950/90 backdrop-blur-xl">
      <nav ref={navRef} className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-5 lg:px-8">
        <Link href="/" onClick={closeMenus} className="group flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <span className="min-w-0">
            <span className="block truncate text-sm font-black uppercase text-white sm:text-base">football-tournament</span>
        ...
        <button
          ...
          className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] text-white transition hover:border-cyan-300 hover:text-cyan-200 lg:hidden"
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`inline-flex items-center gap-1 rounded-lg px-3 py-2 text-sm font-bold transition ${active ? "bg-cyan-300/15 text-cyan-200" : "text-slate-300 hover:bg-white/[0.04] hover:text-cyan-300"}`}
        aria-expanded={open}
      >
        {label}
