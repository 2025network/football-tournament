...
import type { PublicTeam, TeamsResponse, TeamResponse } from "@/types/team";
    ...
    <section className="mx-auto max-w-7xl px-5 py-10 lg:px-8">
      <div className="flex flex-col gap-5 border-b border-white/10 pb-8 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">Create squads, accept invites, and manage members for team tournaments.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="mt-8 rounded-xl border border-white/10 bg-white/[0.03] p-8 text-center"><p className="text-xl font-black text-white">No teams yet</p><p className="mt-2 text-sm text-slate-400">Create a team or ask a captain to invite your account email.</p></div>
      ) : (
          ...
          {teams.map((team) => {
            ...
            return (
              <article key={team.id} className="rounded-xl border border-white/10 bg-white/[0.035] p-5 shadow-[0_0_35px_rgba(14,165,233,0.08)]">
                ...
                {currentMembership?.status === "INVITED" ? <button onClick={() => player && teamAction(team.id, "join", { email: player.email })} className="mt-4 rounded-lg bg-emerald-300 px-4 py-3 text-sm font-black text-slate-950">Accept Invite</button> : null}

                <div className="mt-5 grid gap-2">
                  {team.members.map((member) => <div key={member.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/10 bg-black/20 px-4 py-3 text-sm"><span className="font-bold text-white">{member.gamerTag || member.fullName}</span><span className="text-slate-400">{member.role} - {member.status}</span>{isCaptain && member.role !== "CAPTAIN" ? <button onClick={() => player && teamAction(team.id, "remove-member", { captainEmail: player.email, memberId: member.id })} className="text-rose-300 hover:text-white">Remove</button> : null}</div>)}
                </div>

                {isCaptain ? (
                  <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto_auto]">
                    <input className="form-input" value={inviteEmails[team.id] ?? ""} onChange={(event) => setInviteEmails((current) => ({ ...current, [team.id]: event.target.value }))} placeholder="player@example.com" />
                    <button onClick={() => player && teamAction(team.id, "invite", { captainEmail: player.email, email: inviteEmails[team.id] ?? "" })} className="rounded-lg border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-black text-cyan-100 hover:bg-cyan-300 hover:text-slate-950">Invite</button>
