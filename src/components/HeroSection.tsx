...
export function HeroSection({ title = "Build your profile. Join tournaments. Climb the leaderboard.", subtitle = "A professional esports tournament platform for eFootball Mobile, PUBG Mobile, COD Mobile, and Free Fire players." }: HeroSectionProps) {
  return (
    <section id="home" className="relative isolate overflow-hidden border-b border-white/10">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-73px)] max-w-7xl items-center gap-12 px-5 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
        <div>
          <p className="mb-5 inline-flex rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-cyan-200">Competitive mobile esports</p>
        ...
        <div className="rounded-2xl border border-cyan-300/20 bg-white/[0.06] p-5 shadow-2xl shadow-cyan-950/40 backdrop-blur">
          <div className="rounded-xl border border-white/10 bg-slate-950/80 p-6">
            ...
            <h2 className="mt-2 text-2xl font-black text-white">Your esports identity starts here</h2>
            <div className="mt-6 grid gap-3">
              {["Create account", "Get Platform ID", "Join tournament", "Submit results", "Climb leaderboard"].map((step, index) => (
